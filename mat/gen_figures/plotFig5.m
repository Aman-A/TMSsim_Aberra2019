...
model_prefixes([3,6]) = model_prefixes([6,3]);
% Plot settings
% Threshold plot settings
cmap = [flipud(fake_parula(1000));0.8 0.8 0.8]; % add gray for values above cutoff
clims = [70 230]; % caxis for threshold maps 
cutoff = clims(2)*ones(1,3);
% Threshold diff plot settings
clims2 = [-30 45];
% Sice settings
uvROI = [0.271 0.52 0.42 1]; % FDI ROI for 45deg plane (newer)
% uvROI = [0.271 0.5 0.42 1]; 
plane = [-35.3816  -17.0009 44.3816 -0.5 0.5 0]; % 
normROI = [-40 -35.4 0 0 33.93 45]; % 45 deg slice - posterior part of sulcus plus little lip
start_dim = 2;
ax_zoom = [-38.6 -32.0057 -19.8599 -13.6250 normROI(5)-1 44.6475]; % for plane 45 deg - deeper
% ax_zoom = [-38.6 -32 -19.8599 -14 normROI(5)-1 44.6475]; % for plane 45 deg - deeper
%% Load data
...
num_rows = 2; num_cols = 3; splots = cell(num_rows,num_cols);
...
for i = 1:length(model_prefixes)
    ...
    threshEs_med_all{i} = plotDataSlice(threshEs,cell_model_names,...                           
                        cell_ids,layers,slices,plane,'cutoff',cutoff(1),'above_cutoff_val',1e3);   %,'cutoff',cutoff(1)
   view(ax,plane(4:6));
...
for i = 1:length(model_prefixes)/2
    ...
    threshEsAPi = threshEs_med_all{i+3};
    ...
    for j = 1:5       
       ...
       pi.FaceVertexCData(above_thresh) = 1e3; 
       pi.LineWidth = 1.5;        
   end
    drawnow; 
end
